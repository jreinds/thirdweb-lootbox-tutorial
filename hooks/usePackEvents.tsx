  ...
  useEffect(() => {
    if (provider) {
      ...
      packContract.on("TransferSingle", (_operator, _from, to, _id, _value) => {
        if (to === address) { //the script detects events sent to any address; notify the user only when to equals their address
          toast.success(
            <div className="flex flex-col gap-2">
              <p className="text-green-800">
                {" "}
                Congratulations! You were awarded a generative art NFT pack!
              </p>
              <p>
